...
class GestionDatos {
    ...
    public static function isInTarea($dni,$idTar) {
        self::newConexion();
        $query = "SELECT * FROM tareaUsu WHERE usuario ='".$dni."' AND idTar=".$idTar;
    ...
    public static function addToTarea($dni,$idTar) {
        self::newConexion();
        $query = "INSERT INTO tareaUsu (idTar, usuario) VALUES (" . $idTar . ",'" . $dni ."')";        
        if (self::$conexion->query($query)) {
            echo 'Registro insertado con éxito' . '<br/>';
        } else {
            echo "Error al insertar: " . self::$conexion->error . '<br/>';
    ...
    public static function editTarea($tarea,$desc,$porcDesc,$nivel,$id) {
        self::newConexion();
        $query = "UPDATE tareas SET tarea = '".$tarea."', descripcion = '".$desc."', porcDes =".$porcDesc.", nivel ='".$nivel."' WHERE id = ".$id;
        if (self::$conexion->query($query)) {
            echo "Registro editado Ok";
        } else {
            echo "Error al editar: " . self::$conexion->error .'<br/>';
